...
export function CandidateInterviewPage() {
  ...
  const [phase, setPhase] = useState(searchParams.get("mode") === "tests" ? "tests" : "interview");
  ...
  const sendEvent = useCallback(
    async (eventType, severity = "medium", details = {}) => {
      try {
        ...
        // telemetry errors should not block flow
      }
    },
    [question?.id, token]
  );
    ...
    const timer = setInterval(() => {
      setTimeLeft((value) => {
        if (value <= 1) {
          ...
          void submitAnswerPayload("No answer submitted within time limit.");
          return 0;
        }
        return value - 1;
      });
  ...
  if (!data) {
    return (
      <main className="candidate-shell">
        <div className="panel">Загрузка интервью...</div>
      </main>
    );
  }
    ...
    return (
      <main className="candidate-shell">
        <section className="panel">
          ...
          <p>Ваши результаты уже отправлены HR-команде.</p>
          ...
          <p>Recommendation: {doneScore.recommendation}</p>
          <Link className="secondary-btn" to="/">
            На главную SkillGraph
          </Link>
        </section>
      </main>
    );
  ...
  if (phase === "tests") {
    return (
      <main className="candidate-shell">
        <section className="panel">
          <h2>Тестирование кандидата</h2>
          <p>Пройдите все тесты перед финальной отправкой интервью.</p>
          {tests.map((test) => (
            <article className="test-card" key={test.id}>
              <header>
                <h3>{test.title}</h3>
                <span>{Math.round(test.duration_sec / 60)} мин</span>
              </header>
              ...
              <button className="secondary-btn" type="button" onClick={() => submitTest(test)}>
                {submittedTests[test.id]
                  ? `Отправлено (${submittedTests[test.id].normalized_score.toFixed(1)}%)`
                  : "Отправить тест"}
              </button>
            </article>
          ))}
  ...
  if (!question) {
    return (
      <main className="candidate-shell">
        <section className="panel">Загрузка...</section>
      </main>
    );
  }
    ...
    <main className="candidate-shell">
      <section className="panel">
        ...
        <div className="chip-row">
          <span className={`chip ${timeLeft < 30 ? "warning" : ""}`}>Осталось: {timeLeft} сек</span>
          <span className="chip">Тип: {question.question_type}</span>
        </div>
        <h2>{question.text}</h2>
        <div className="actions-row">
          ...
          <button className="secondary-btn" onClick={toggleRecording} type="button">
            {recording ? "Стоп запись" : "Записать голосовой ответ"}
          </button>
        </div>
